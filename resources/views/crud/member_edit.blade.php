...
<body>
    <div class="p-6 text-gray-900">
        <form method="POST" action="{{ route('membercrud.update', $member->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="instrument" id="instrument" value="{{ old('instrument', $member->instrument) }}" required>
                @error('instrument')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="year" id="year" value="{{ old('year', $member->year) }}" required>
                @error('year')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
                ...
                <select name="artist_id" id="artist_id" class="mt-1 block w-full rounded border-gray-300 shadow-sm">
                    @foreach($artists as $artist)
                        @if($artist->is_band === 'yes')
                            <option value="{{ $artist->id }}" {{ $member->artist_id == $artist->id ? 'selected' : '' }}>
                                {{ $artist->name }}
                            </option>
                        @endif
                    @endforeach
            ...
            <div class="mb-4">
                ...
                <input type="text" name="image" id="image" value="{{ old('image', $member->image) }}">
                @error('image')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
